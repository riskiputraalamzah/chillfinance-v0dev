...
const validateUsername = (username) => {
  ...
  if (!/^[A-Za-z0-9_\-\s]+$/.test(username)) {
    return { valid: false, error: "Username hanya boleh berisi huruf, angka, spasi, _ atau -." }
  }
...
document.getElementById("register-form")?.addEventListener("submit", (e) => {
  ...
  if (StorageManager.getUser(username)) {
    document.getElementById("username-error").textContent = "Username sudah terdaftar."
    return
  }
...
function updateAnalytics() {
  ...
  document.getElementById("total-nabung").textContent = formatRupiah(totalNabung)
...
function renderDashboardTargets() {
  ...
  if (Object.keys(targets).length === 0) {
    container.innerHTML = '<p class="riwayat-empty">Belum ada target tabungan. Buat yang pertama yuk! 🎯</p>'
    return
  }
...
function createTargetElement(name, target) {
  const percentage = Math.min(100, Math.round((target.saldo / target.target) * 100))
  ...
  return `
        <div class="target-item">
            ...
            <div class="target-item-info">
                <span>${formatRupiah(target.saldo)} / ${formatRupiah(target.target)}</span>
...
function populateTargetSelects() {
  ...
  if (nabungSelect) {
    nabungSelect.innerHTML =
      ...
      activTargets.map((name) => `<option value="${name}">${name}</option>`).join("")
  }
...
document.getElementById("add-target-form")?.addEventListener("submit", (e) => {
  ...
  const nominal = Number.parseInt(document.getElementById("target-nominal").value)

  if (Object.keys(currentUser.targets).some((n) => n.toLowerCase() === name.toLowerCase())) {
...
function renderTargets() {
  ...
  if (Object.keys(currentUser.targets).length === 0) {
    container.innerHTML = '<p class="riwayat-empty">Belum ada target. Buat yang pertama yuk! 🎯</p>'
    return
  }
...
document.getElementById("nabung-jumlah")?.addEventListener("input", (e) => {
  ...
  document.getElementById("format-nabung").textContent = value ? formatRupiah(value) : "Rp 0"
})

document.getElementById("nabung-catatan")?.addEventListener("input", (e) => {
  const count = e.target.value.length
  e.target.parentElement.querySelector(".count-text").textContent = `${count}/120`
})

document.getElementById("nabung-form")?.addEventListener("submit", (e) => {
  e.preventDefault()
  const sumber = document.getElementById("nabung-sumber").value
  const jumlah = Number.parseInt(document.getElementById("nabung-jumlah").value)
  const catatan = document.getElementById("nabung-catatan").value || "-"

  if (sumber === "utama") {
    currentUser.saldo_utama += jumlah
    currentUser.riwayat.push([new Date().toLocaleString("id-ID"), "nabung", jumlah, catatan])
    ...
    target.saldo += jumlah
    target.riwayat.push([new Date().toLocaleString("id-ID"), "nabung", jumlah, catatan])
    ...
    if (target.saldo >= target.target) {
      ...
      alert(`✅ Nabung Rp ${formatRupiah(jumlah)} ke target '${targetName}' berhasil!`)
    }
  }
...
document.getElementById("pengeluaran-sumber")?.addEventListener("change", (e) => {
...
document.getElementById("pengeluaran-jumlah")?.addEventListener("input", (e) => {
  const value = e.target.value
  document.getElementById("format-pengeluaran").textContent = value ? formatRupiah(value) : "Rp 0"
})

document.getElementById("pengeluaran-catatan")?.addEventListener("input", (e) => {
  const count = e.target.value.length
  e.target.parentElement.querySelector(".count-text").textContent = `${count}/120`
})
...
document.getElementById("pengeluaran-form")?.addEventListener("submit", (e) => {
  ...
  const jumlah = Number.parseInt(document.getElementById("pengeluaran-jumlah").value)
  ...
  if (sumber === "utama") {
    ...
    currentUser.riwayat.push([new Date().toLocaleString("id-ID"), "keluar", jumlah, catatan])
    alert(`✅ Pengeluaran Rp ${formatRupiah(jumlah)} dari saldo utama dicatat.`)
  } else {
    ...
    const target = currentUser.targets[targetName]
    const now = new Date()
    const lastWithdraw = target.last_withdraw ? new Date(target.last_withdraw) : null

    if (lastWithdraw) {
      const daysDiff = Math.floor((now - lastWithdraw) / (1000 * 60 * 60 * 24))
      if (daysDiff < 365) {
        const sisaHari = 365 - daysDiff
        alert(`❌ Anda sudah melakukan penarikan tahun ini. Coba lagi dalam ${sisaHari} hari.`)
        return
      }
    }
    ...
    if (!confirm(`⚠️ Penarikan maksimal 30% = Rp ${formatRupiah(maxTarik)}. Lanjutkan?`)) {
      return
    }

    target.saldo -= maxTarik
    target.last_withdraw = now.toLocaleString("id-ID")
    target.riwayat.push([now.toLocaleString("id-ID"), "keluar", maxTarik, catatan])
    alert(`✅ Penarikan Rp ${formatRupiah(maxTarik)} dari target '${targetName}' berhasil!`)
  }
...
function renderUtamaRiwayat() {
  ...
  if (currentUser.riwayat.length === 0) {
    container.innerHTML = '<p class="riwayat-empty">Belum ada riwayat transaksi.</p>'
    return
  }
      ...
      ([tanggal, tipe, jumlah, catatan]) => `
            <div class="riwayat-item ${tipe}">
                <div class="riwayat-info">
                    <div class="riwayat-title">${tipe === "nabung" ? "➕ Nabung" : "➖ Pengeluaran"}</div>
                    <div class="riwayat-time">${tanggal}</div>
                    ${catatan !== "-" ? `<div class="riwayat-note">${catatan}</div>` : ""}
                </div>
                <div class="riwayat-amount ${tipe}">${tipe === "nabung" ? "+" : "-"} ${formatRupiah(jumlah)}</div>
            </div>
        `,
    )
    .join("")
...
function renderTargetRiwayat() {
  ...
  if (targets.length === 0) {
    container.innerHTML = ""
    riwayatContainer.innerHTML = '<p class="riwayat-empty">Belum ada target.</p>'
    return
  }
  ...
  container.innerHTML = targets
    .map(
      (name, idx) => `
            <button class="target-selector-btn ${idx === 0 ? "active" : ""}" onclick="showTargetRiwayat('${name}')">
                ${name}
            </button>
        `,
    )
...
function showTargetRiwayat(targetName) {
  ...
  if (!target.riwayat || target.riwayat.length === 0) {
    container.innerHTML = '<p class="riwayat-empty">Belum ada riwayat transaksi pada target ini.</p>'
    return
  }
  ...
  container.innerHTML = target.riwayat
    ...
    .map(
      ([tanggal, tipe, jumlah, catatan]) => `
            <div class="riwayat-item ${tipe}">
                <div class="riwayat-info">
                    <div class="riwayat-title">${tipe === "nabung" ? "➕ Nabung" : "➖ Penarikan"}</div>
                    <div class="riwayat-time">${tanggal}</div>
                    ${catatan !== "-" ? `<div class="riwayat-note">${catatan}</div>` : ""}
                </div>
                <div class="riwayat-amount ${tipe}">${tipe === "nabung" ? "+" : "-"} ${formatRupiah(jumlah)}</div>
